...
int get_volume(const short *pcmdata, size_t size)
{
    ...
    if (db > max_volume)
        max_volume = db;
    // printf("volume db: %.5d max: %.5d len:%d\n", db,max_volume,len);
    return db;
}
...
void OL_WKUP::deal_volume_flag(const short *pcmdata)
{
    ...
    if (volume < 2000)
    {
        if (timer_2.has_pass(1) && !AudioRec::slient_flag) {
            ...
            AudioRec::start_record = 0;  // 结束录音
            // printf("start_record = 0\n");
            // printf("slient_flag = 1\n\n");
        }
    } else {
        AudioRec::slient_flag = 0;
        // printf("slient_flag = 0\n\n");
        timer_2.restart();
    }
    // 检测语音
    if (volume > volume_ok)
    {
        ...
        if (!AudioRec::start_record)
        {
            ...
            AudioRec::slient_flag = 0;
            // printf("start_record = 1\n");
            // printf("slient_flag = 0\n\n");
        }
    }

    printf("volume %d slient %d start %d \n", volume, AudioRec::slient_flag, AudioRec::start_record);
}
